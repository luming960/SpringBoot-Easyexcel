...
import com.wlb.file.util.UserUtils;
...
public class UserController {
    ...
    //跳转到用户表上传页面，上传页面可上传本机excel，将表格里面的数据存储到数据库
    ...
    public String export2File() {
        UserUtils.export2File(path, "用户表", "用户信息", User.class, userService.getAll());
        return "导出成功";
    }

    // easyexcel获取数据库数据并导出Excel到浏览器下载下来
    @GetMapping("/exportWeb")
    public void export2Web(HttpServletResponse response) {
        try {
            UserUtils.export2Web(response, "用户表", "用户信息", User.class, userService.getAll());
        } catch (Exception e) {
            log.error("报表导出异常:", e);
        }
    }
    ...
    public String export2Web4File(HttpServletResponse response, @PathVariable String excelName) {
        try {
            return UserUtils.export2Web4File(response, path, excelName);
        } catch (Exception e) {
            log.error("文件导出异常：", e);
        }
        return "文件导出成功";
